...
//		Should have invites and at least one calendar. Therefore not groups
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, loggedInUser.getCalendarIds().isEmpty());
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, loggedInUser.getInviteIds().isEmpty());
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, loggedInUser.getGroupIds().isEmpty());
...
//		request own user object completely
		...
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, userRequest.getCalendarIds().isEmpty());
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, userRequest.getInviteIds().isEmpty());
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, userRequest.getGroupIds().isEmpty());
...
//		Request another user by its id
		...
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, userRequest2.getCalendarIds().isEmpty());
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, userRequest2.getInviteIds().isEmpty());
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, userRequest2.getGroupIds().isEmpty());

//		request another user's calendar (calendar from the top of this method is used)
		assertNull(ERROR_TYPE_REQ + ERROR_FORBIDDEN + NOTNULL_CALENDAR, requestCalendar(calendarUser2.getID()));
//		request another user's invite you have not send (User 1 invites just User 2)
		assertNull(ERROR_TYPE_REQ + ERROR_FORBIDDEN + NOTNULL_INVITE, requestInvite(userList[2].getInviteIds().get(0)));
//		request another user's entry you are not invited to (User 2 invites User 3)
		...
		assertNull(ERROR_TYPE_REQ + ERROR_FORBIDDEN + NOTNULL_ENTRY, requestCalendarEntry(noInvite.getID()));
...
//		This method uses the requestObjectByOwnID() --> testRequestObjectByOwnId() must be successful
	...
	public void testRequestAllObjects() {
		...
		for(User u: user){
			assertTrue(ERROR_TYPE_LIST + LIST_EMPTY, u.getCalendarIds().isEmpty());
		...
		for(int i = 0; i < user2.size(); i++){
			assertTrue(ERROR_TYPE_LIST + LIST_EMPTY, user2.get(i).getCalendarIds().isEmpty());
			assertTrue(ERROR_TYPE_LIST + LIST_EMPTY, user2.get(i).getInviteIds().isEmpty());
			if(i == 0 || i == 1){
				assertFalse(ERROR_TYPE_LIST + LIST_FILLED, user2.get(i).getGroupIds().isEmpty());
			}else{
				assertTrue(ERROR_TYPE_LIST + LIST_EMPTY, user2.get(i).getGroupIds().isEmpty());
			}
		}
	}
	...
	public void testRequestAllObjectsByProperty() {
		...
		requestEntry.setRequestProperty(CalendarEntry.OWNER_ID, ""+loggedInUser.getID());
		List<CalendarEntry> entries = requester.requestAllObjectsByProperty(requestEntry);
		...
		List<CalendarEntry> entries2 = requester.requestAllObjectsByProperty(requestEntry2);
		...
		requestGroup.setRequestProperty(Group.USER_ID, ""+loggedInUser.getID());
		List<Group> groups2 = requester.requestAllObjectsByProperty(requestGroup);
		assertNotNull(groups);
		assertFalse(groups.isEmpty());
...
//		Add calendar to another user should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_CALENDAR, addCalendar(user1.getGivenName(), user2.getID()));
...
//		Add calendar to another user should fail		
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_CALENDAR, addCalendar(user2.getGivenName(), user1.getID()));
...
//		Add entry to another user should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser1ToUser1.getID(), user2.getID(), 0));
//		Add entry to another user's calendar should fail		
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser2ToUser2.getID(), user1.getID(), 0));
//		Add entry to another user's calendar and user itself should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser2ToUser2.getID(), user2.getID(), 0));
...
//		Add entry to another user should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser1ToUser1.getID(), user2.getID(), room1.getID()));
//		Add entry to another user's calendar should fail		
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser2ToUser2.getID(), user1.getID(), room1.getID()));
//		Add entry to another user's calendar and user itself should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user1.getGivenName(), calendarUser2ToUser2.getID(), user2.getID(), room1.getID()));
...
//		Add entry to another user should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser2ToUser2.getID(), user1.getID(), 0));
//		Add entry to another user's calendar should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser1ToUser1.getID(), user2.getID(), 0));
//		Add entry to another user's calendar and user itself should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser1ToUser1.getID(), user1.getID(), 0));
...
//		Add entry to another user should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser2ToUser2.getID(), user1.getID(), room1.getID()));
//		Add entry to another user's calendar should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser1ToUser1.getID(), user2.getID(), room1.getID()));
//		Add entry to another user's calendar and user itself should fail
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_ENTRY, addCalendarEntry(user2.getGivenName(), calendarUser1ToUser1.getID(), user1.getID(), room1.getID()));
...
//		Add invite with no room set
		...
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser2ToUser1sEntryNoRoom);
//		Add invite with room set
		Invite inviteUser2ToUser1sEntryWithRoom1 = addInvite(entryUser1ToUser1WithRoom1.getID(), user2.getID());
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser2ToUser1sEntryWithRoom1);
//		Add second invite with room set
		...
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser2ToUser1sEntryWithRoom2);
		
//		Add invite for other user to its own calendar entry	without room		
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser2ToUser2NoRoom.getID(), user2.getID()));
//		Add invite for other user to its own calendar entry	with room
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser2ToUser2WithRoom1.getID(), user2.getID()));
//		Add own invite for other users calendar entry
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser2ToUser2WithRoom2.getID(), user1.getID()));
		
		login(user2);
//		Add invite with no room set
		Invite inviteUser1ToUser2sEntryNoRoom = addInvite(entryUser2ToUser2NoRoom.getID(), user1.getID());
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser1ToUser2sEntryNoRoom);
//		Add invite with room set
		Invite inviteUser1ToUser2sEntryWithRoom1 = addInvite(entryUser2ToUser2WithRoom1.getID(), user1.getID());
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser1ToUser2sEntryWithRoom1);
//		Add second invite with room set
		Invite inviteUser1ToUser2sEntryWithRoom2 = addInvite(entryUser2ToUser2WithRoom2.getID(), user2.getID());
		assertNotNull(ERROR_TYPE_ADD + NULL_INVITE, inviteUser1ToUser2sEntryWithRoom2);
		
//		Add invite for other user to its own calendar entry	without room		
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser1ToUser1NoRoom.getID(), user1.getID()));
//		Add invite for other user to its own calendar entry	with room
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser1ToUser1WithRoom1.getID(), user1.getID()));
//		Add own invite for other users calendar entry
		assertNull(ERROR_TYPE_ADD + ERROR_FORBIDDEN + NOTNULL_INVITE, addInvite(entryUser1ToUser1WithRoom2.getID(), user2.getID()));
...
//		Check whether still invites exist 
		assertTrue(ERROR_TYPE_LIST + LIST_FILLED, loggedInUser.getInviteIds().isEmpty());
...
//		Remove other user's Invites - User 3, because User 2 has no invites anymore
...
//		Remove other user's Entry
		assertFalse(ERROR_TYPE_REM + ERROR_FORBIDDEN + TRUE_ENTRY, removeCalendarEntry(7));
		
//		Remove other user's Calendar	
		assertFalse(ERROR_TYPE_REM + ERROR_FORBIDDEN + TRUE_CALENDAR, removeCalendar(calendarUser2.getID()));
		
//		Remove other user
		assertFalse(ERROR_TYPE_REM + ERROR_FORBIDDEN + TRUE_USER, removeUser(testUser[1].getID()));
		
//		Login User 2
		loggedInUser = login(testUser[1].getMailAddress());
//		Request Calendar
		Calendar calendar2 = requestCalendar(loggedInUser.getCalendarIds().get(0));
...
//		request all invites of user
		List<Invite> invites = requester.requestFollowingObjectsByOwnId(loggedInUser.getInviteIds(), new Invite());
//		List is not null
...
//		all invites were accepted --> invites.size + oldEntry.size == newEntry.size
		assertTrue(ERROR_TYPE_ADD + NULL_ENTRY, (entrySize+loggedInUser.getInviteIds().size())==entrySize2);
...
//		double accpeting an invite should fail
		List<Invite> invites2 = requester.requestFollowingObjectsByOwnId(loggedInUser.getInviteIds(), new Invite());
//		List is not null
...
//		Request Invites of User 2
		List<Invite> invitesUser2 = requester.requestFollowingObjectsByOwnId(loggedInUser.getInviteIds(), new Invite());
...
//		all invites were accepted --> invites.size + oldEntry.size == newEntry.size
		assertTrue(ERROR_TYPE_ADD + NULL_ENTRY, (entrySize2+loggedInUser.getInviteIds().size())==entrySize3);
...
//		Update other user's Entry
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, calendarUser2.getCalendarEntries().isEmpty());
		assertFalse(ERROR_TYPE_UPD + ERROR_FORBIDDEN + TRUE_ENTRY, updateCalendarEntry(calendarUser2.getCalendarEntries().get(0)));
		
//		Update other user's Calendar	
		assertFalse(ERROR_TYPE_UPD + ERROR_FORBIDDEN + TRUE_CALENDAR, updateCalendar(calendarUser2.getID()));
		
//		Update other user
		assertFalse(ERROR_TYPE_UPD + ERROR_FORBIDDEN + TRUE_USER, updateUser(testUser[2].getID()));
...
//		Update User 2 to same Mailaddress should fail
	...
	public void testAddUserToGroup() {
		...
		for(Group group: groups){
			boolean result = requester.addUserToGroup(loggedInUser, group);
			assertFalse(ERROR_TYPE_ADD + ERROR_FORBIDDEN_TWICE + USER_GROUP_SUCCESS, result);
		}
		
		for(Group group: groups){
			boolean result = requester.addUserToGroup(userList[1], group);
			assertFalse(ERROR_TYPE_ADD + ERROR_FORBIDDEN + USER_GROUP_SUCCESS, result);
		}
	}
...
//			MemberList should be empty with no user was added up to now
			assertTrue(ERROR_TYPE_LIST + LIST_FILLED, group.getMemberIds().isEmpty());
		}
...
//		List should be filled with users now
		...
		for(Group group: groups2){
			boolean resultRemoveOtherUser = requester.removeUserFromGroup(userList[2], group);
			assertFalse(ERROR_TYPE_REM + ERROR_FORBIDDEN + USER_GROUP_SUCCESS, resultRemoveOtherUser);
			
			boolean resultRemoveOwnUser = requester.removeUserFromGroup(loggedInUser, group);
...
//		List should be still filled with users
		assertFalse(ERROR_TYPE_LIST + LIST_EMPTY, groups2.isEmpty());
		for(Group group: groups2){
			boolean resultUserStillInGroup = group.getMemberIds().contains(loggedInUser.getID());
...
//	####################################
	...
	private CalendarEntry addDefaultEntry(User user, String suffix, int roomId){
		...
		calendarEntry.setDescription("!\"$%&/()[]*+'#,;:=><ÄÖÜäöüß Entry automated generated for "+user.getGivenName() + " " + user.getSurname());
		calendarEntry.setStartDate(new Date());
		calendarEntry.setEndDate(new Date());
